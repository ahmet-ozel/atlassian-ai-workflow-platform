...
class TestModuleLevelTaskQueueConstant:
    ...
    def test_constant_resolves_via_registry(self) -> None:
        ...
        assert main_mod.AGENT_RUNNER_TASK_QUEUE == task_queue_for(
            "AgentRunnerWorkflow"
        )
    ...
    def test_constant_value_is_agent_runner_tq(self) -> None:
        ...
        assert main_mod.AGENT_RUNNER_TASK_QUEUE == "agent-runner-tq"
...
def _parse_main_module() -> ast.Module:
    """Return the parsed AST of ``agent_runner.main``."""

    main_path = _SRC_DIR / "agent_runner" / "main.py"
...
class TestWorkerConstructorCallSite:
    """The ``Worker(...)`` call must use ``task_queue_for("AgentRunnerWorkflow")``
    ...
    def test_exactly_one_worker_constructor_call(self) -> None:
        ...
        assert len(worker_calls) == 1, (
            "agent-runner-worker boot script must construct exactly "
            "one Worker (one queue per worker); found "
            f"{len(worker_calls)}."
        )
    ...
    def test_task_queue_kwarg_resolves_via_registry(self) -> None:
        ...
        if isinstance(value, ast.Call):
            _assert_task_queue_for_call(value, "AgentRunnerWorkflow")
...
class TestWorkerWorkflowsRegistration:
    ...
    def test_workflows_kwarg_registers_agent_runner_workflow(self) -> None:
        ...
        assert "AgentRunnerWorkflow" in names, (
            "agent-runner-worker must register AgentRunnerWorkflow on "
            f"the agent-runner-tq queue; got {sorted(names)}"
        )
