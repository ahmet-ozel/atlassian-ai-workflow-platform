...
is exercised against every ops action listed above.
"""
...
#: test broadens to all RBAC roles since governs *who*
...
#: Mandatory ``chat_message`` payload fields. The validator below rejects events that
 ...
 """
    ...
    if null_fields:
        raise ValueError(
            ...
            "carry a populated value (the operational rule)."
        )
...
def _payload_for_system_action(action: str) -> st.SearchStrategy[dict[str, Any]]:
 ...
 pinned by the unit tests that own each emitter (invariant is the
    ...
    for row in writer.inserted:
        if row.action in _SYSTEM_ACTIONS:
            assert row.actor_role == "system", (
                ...
                "system actions must use the synthetic system role"
            )
...
def test_ops_audit_rejects_null_or_invalid_actor_role(
 ...
 Foundation invariant parity: the ``actor_role IS NOT NULL``
 ...
 """
    ...
    async def run() -> None:
        ...
        assert (
            ...
            "the operational rule so operators can pivot from the traceback"
        )
...
def test_ops_universe_includes_all_required_actions() -> None:
    """Concrete anchor: the test's action universe matches required actions.

 This test pins the ops action list so future action changes are
 reflected in the strategy without silent drift.
 """
