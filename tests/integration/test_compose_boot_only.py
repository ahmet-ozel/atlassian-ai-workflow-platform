...
**Property 2: Compose Boot-Only Set**

**Validates: Requirements 2.1, 2.2, 2.6**

Spec: ``.kiro/specs/platform-real-usage-gaps`` (task 2.4).

What this test asserts
----------------------

R2.1 says ``make boot`` MUST execute ``docker compose -f
...
R2.2 keeps ``make up`` as an alias for ``make boot`` and introduces
...
R2.6 codifies the invariant as a CI test: ``docker compose -f
...
* It does not bring containers up — that is task 14.3 / the existing
  ...
  smoke tests. The R2.6 assertion is purely about which services
...
#: The dev override layered by ``make boot``. R2.1's ``COMPOSE_BOOT``
...
#: The boot bundle defined by R2.1 / R2.6. Exactly these four services
...
#: Services R2.6 explicitly calls out as profile-gated. They MUST be
...
def test_profile_less_compose_yaml_only_lists_the_boot_bundle(
    ...
    This is the always-on path: it does not need a Docker daemon,
    ...
    # ---- Primary assertion (R2.1, R2.6): exact equality ----
    ...
    assert boot_services == EXPECTED_BOOT_BUNDLE, (
        ...
        f"bundle {sorted(EXPECTED_BOOT_BUNDLE)} (Requirement 2.1 / "
        "Property 2).\n"
    ...
    # ---- Secondary assertion (R2.6 explicit list): named services
    ...
    assert not not_declared, (
        "the following services are named in R2.6 as profile-gated "
    ...
    assert not leaked_into_default, (
        "the following services MUST stay profile-gated per R2.6 but "
    ...
    assert not weakly_gated, (
        ...
        f"missing `profiles:` list: {weakly_gated}. R2.6 requires "
...
def test_profile_less_compose_config_services_matches_boot_bundle(
    ...
    This is the higher-fidelity sibling of the YAML-only test: it
    ...
    assert compose_file.is_file(), (
        ...
        f"the boot bundle invariant (R2.6)."
    )
    ...
    # what R2.6 specifies. Layering the dev override here would also
    ...
    # And the explicitly-named profile-gated services from R2.6 MUST
