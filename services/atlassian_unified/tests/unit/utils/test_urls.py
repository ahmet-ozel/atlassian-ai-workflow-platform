...
class TestResolveRelativeUrl:
    ...
    # NOTE: do NOT name the second parameter ``base_url`` — that collides
    # with the session-scoped ``base_url`` fixture provided by pytest-base-url
    # which is pulled in transitively, and causes a ``ScopeMismatch`` at setup.
    @pytest.mark.parametrize(
        ("url", "service_base_url", "expected"),
        [
    ...
    def test_resolve_relative_url(
        self, url: str, service_base_url: str, expected: str
    ) -> None:
        """Parametrized test for resolve_relative_url."""
        assert resolve_relative_url(url, service_base_url) == expected
