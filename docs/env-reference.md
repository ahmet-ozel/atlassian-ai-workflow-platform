...
| `MAIL_SESSION_VAULT_BACKEND` | `assistant-service`, `gmail-mcp`, `outlook-mcp` | false | `hashicorp` | Mail session credential hattının backend seçimi; local geliştirmede `local-dev` seçilirse Gmail/Outlook OAuth materyali encrypted named volume'da restart'a dayanıklı saklanır. | - | false |
| `MAIL_SESSION_VAULT_LOCAL_KEY` | `assistant-service`, `gmail-mcp`, `outlook-mcp` | true when `MAIL_SESSION_VAULT_BACKEND=local-dev` | (boş) | Local-dev mail credential store için 32 byte hex/base64 libsodium anahtarı; repo'ya commit edilmez, sadece yerel `.env` içinde tutulur. | `vault:infrastructure/vault/local_dev_key` | false |
| `MAIL_SESSION_VAULT_LOCAL_STORE` | `assistant-service`, `gmail-mcp`, `outlook-mcp` | false | `/vault-local/vault.json` | Mail session credential'larının local-dev backend altında yazıldığı encrypted store dosyası. | - | false |
