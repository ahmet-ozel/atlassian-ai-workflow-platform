"""Notification dispatch activity for workflow completion (R5.2 / R5.3).

Wires the ``automation-worker`` Temporal activity surface to
:class:`notification.NotificationService` so a finishing workflow
(``AutomationWorkflow`` / ``IterationWorkflow`` / future siblings)
can fan out a Slack / Email completion notification per the decision
table in ``platform-mimari-ops`` design.md §`NotificationService`:

* ``status == "failed"`` ⇒ Slack send is **mandatory** regardless of
  ``dept.notify_on_success`` (R5.3).
* ``status ∈ {"completed", "partial"}`` ⇒ success-gated; the dispatcher
  short-circuits to a no-op when ``notify_on_success == False`` (R5.2).
...
* :func:`dispatch_notification` — the primary call.  Builds a
  :class:`notification.DeptConfigView` and
  :class:`notification.WorkflowResult` from the flat
  :class:`DispatchNotificationInput` payload, then forwards to
  :meth:`NotificationService.notify_workflow_completion`.
...
PR).  Every swallowed exception is logged via
:data:`activity.logger` so an operator chasing a missing notification
...
:mod:`automation_worker.activities.audit_prune`:
...
* :func:`set_notification_service` / :func:`get_notification_service`
  ...
  startup.  Tests inject an in-memory fake via
  :func:`set_notification_service`.

The module-level state is **separate** from
:mod:`automation_worker.activities.audit_prune` (which has its own
...
class DispatchNotificationInput:
    ...
    The dataclass is deliberately **flat** (no nested
    :class:`notification.DeptConfigView` / :class:`notification.WorkflowResult`
    ...
    Args:
        ...
        dept_id: Stable department slug; surfaced into the
            :class:`notification.DeptConfigView`.  Used as an audit
        ...
        notify_channels: Tuple of channel names the department
            ...
            of this set (R5.3).  Tuple (not frozenset) for Temporal
        ...
        status: Terminal workflow status.  One of ``"completed"`` /
            ...
            failure-mandatory branches inside
            :meth:`NotificationService.notify_workflow_completion`.
...
class _NotificationServiceLike(Protocol):
    ...
    the audit-prune admin alarm is wired through
    :mod:`automation_worker.activities.audit_prune` against the same
...
def set_notification_service(service: _NotificationServiceLike) -> None:
    ...
    call this with an in-memory fake whose
    :meth:`notify_workflow_completion` records its inputs.
    """
...
async def dispatch_notification(inp: DispatchNotificationInput) -> None:
    ...
    notification transport hiccup.  Idempotency is enforced inside
    :class:`NotificationService` (the deterministic ``dedup_key``
    ...
    3. Builds the typed value objects from the flat
       :class:`DispatchNotificationInput` payload.
    ...
    except Exception as exc:  # noqa: BLE001 — best-effort (R5.2 / R5.3)
        activity.logger.warning(
