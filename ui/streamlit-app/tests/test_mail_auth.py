...
def test_mail_auth_statuses_keep_oauth_in_mcp_services() -> None:
    ...
    assert all(status.token_storage == "per-user-vault-credential-ref" for status in statuses)


def test_gmail_oauth_env_contract_has_no_mail_secret_requirements() -> None:
    status = mail_auth_status("gmail")

    assert status.required_env_keys == ()


def test_outlook_oauth_env_contract_has_no_mail_secret_requirements() -> None:
