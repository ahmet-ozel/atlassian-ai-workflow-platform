# vps_credential_loader.ps1 â€” Generate platform/.env + services/atlassian_mcp_bitbucket/.env
# Covers platform and VPS credential loading checks.
#
# Usage: . .\vps_credential_loader.ps1
#   Produces platform/.env and services/atlassian_mcp_bitbucket/.env locally,
...
# =============================================================================
# STEP 2: Read .env template keys
# =============================================================================
Write-Host "[credential_loader] Reading .env template..." -ForegroundColor Cyan

if (-not (Test-Path $EnvExampleFile)) {
    throw ".env not found at: $EnvExampleFile"
}
...
# =============================================================================
# STEP 4: Build .env content â€” superset of .env keys (R4.1)
# =============================================================================
...
$envContent = @"
# =============================================================================
# platform/.env â€” Generated by vps_credential_loader.ps1
...
# --- Feature flags (R4.5 â€” all disabled for E2E test) ---
...
Invoke-VpsSsh "mkdir -p /opt/atlassian-ai-workflow-platform"

# Copy .env file
Copy-ToVps -LocalPath $EnvOutputFile -RemotePath "/opt/atlassian-ai-workflow-platform/.env"

# Set restrictive permissions
Invoke-VpsSsh "chmod 600 /opt/atlassian-ai-workflow-platform/.env"

Write-Host "[credential_loader] .env deployed to VPS with chmod 600" -ForegroundColor Green

# =============================================================================
# STEP 8: Generate evidence file â€” KEY NAMES ONLY, no values (R22)
# =============================================================================
...
# Generated: $(Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ")
# Requirement: R4.1 (superset of .env), R22 (no secrets in evidence)
...
# =============================================================================
Write-Host "[credential_loader] Validating .env superset coverage..." -ForegroundColor Cyan
...
if ($missingKeys.Count -gt 0) {
    Write-Host "[credential_loader] WARNING: Missing keys from .env:" -ForegroundColor Yellow
    ...
    Write-Host "[credential_loader] All .env keys present in generated .env (R4.1 satisfied)" -ForegroundColor Green
}

# =============================================================================
# STEP 10: Generate services/atlassian_mcp_bitbucket/.env
# =============================================================================
...
$mcpEnvContent = @"
# =============================================================================
# services/atlassian_mcp_bitbucket/.env â€” Generated by vps_credential_loader.ps1
# Generated: $(Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ")
# WARNING: This file contains secrets. NEVER commit to git.
# =============================================================================
...
# --- Bitbucket â€” Token B (Basic Auth, primary) (R5.2) ---
...
# Ensure remote directory exists
Invoke-VpsSsh "mkdir -p /opt/atlassian-ai-workflow-platform/services/atlassian_mcp_bitbucket"

# Copy MCP .env file
Copy-ToVps -LocalPath $McpEnvOutputFile -RemotePath "/opt/atlassian-ai-workflow-platform/services/atlassian_mcp_bitbucket/.env"

# Set restrictive permissions
Invoke-VpsSsh "chmod 600 /opt/atlassian-ai-workflow-platform/services/atlassian_mcp_bitbucket/.env"

Write-Host "[credential_loader] MCP .env deployed to VPS with chmod 600" -ForegroundColor Green

# =============================================================================
# STEP 12: Generate MCP evidence file â€” KEY NAMES ONLY (R22)
# =============================================================================
...
$mcpEvidenceContent = @"
# Evidence: services/atlassian_mcp_bitbucket/.env key inventory (values redacted for security)
# Generated: $(Get-Date -Format "yyyy-MM-ddTHH:mm:ssZ")
...
# =============================================================================
# STEP 13: Run property test â€” test_env_coverage.py (R4.6)
# =============================================================================
Write-Host "[credential_loader] Running property test: test_env_coverage.py..." -ForegroundColor Cyan

try {
    $pytestOutput = Invoke-VpsSsh "cd /opt/atlassian-ai-workflow-platform && python3 -m pytest tests/property/test_env_coverage.py -v 2>&1"
    $pytestExitCode = 0
...
Write-Host "  Remote:   /opt/atlassian-ai-workflow-platform/.env (chmod 600)" -ForegroundColor White
Write-Host "  MCP Local:  $McpEnvOutputFile" -ForegroundColor White
Write-Host "  MCP Remote: /opt/atlassian-ai-workflow-platform/services/atlassian_mcp_bitbucket/.env (chmod 600)" -ForegroundColor White
