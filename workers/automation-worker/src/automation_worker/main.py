 ...
 * The worker hosts the expected workflows on one queue.
 * The daily cron archives ``audit_events`` older than
 ``RETENTION_DAYS`` to MinIO and then deletes them.
 * Any failure invokes the mandatory admin Slack alarm
 via the ``notify_audit_prune_failed`` activity.
 * The branch-retention cron deletes stale ``ai/*`` branches.
"""
...
async def _build_postgres_pool() -> object:
 ...
 ``asyncpg`` dependency (the test suite stubs the pool via:func:`audit_prune_activities.set_db_pool`).
 """
 ...
 * The:class:`prompts.PromptLoader` from the boot script is
 ...
 """
    ...
    # day. The wrapper service below builds the session on demand
...
def _build_credential_resolver() -> Any:
 ...
 Used by the production HTTP-backed MCP caller so each ``execute_output_actions``
...
class _LazyNotificationService:
 ...
 and the notification log store, then build the real:class:`NotificationService` and forward the call.
 ...
 Mirrors:meth:`notify_audit_prune_failed`: the underlying:class:`NotificationService` is built on first use (shared
...
def _audit_prune_prompt_renderer() -> object:
 ...
 The full:class:`prompts.PromptLoader` is wired in:mod:`assistant_service` for hot-reload; the worker only needs
...
async def _run_async() -> None:
    ...
    # Wire the production HTTP-based MCP caller so every outbound MCP request
    # from the ``execute_output_actions`` activity carries
    # ``X-Client-Source: automation-worker``. The caller resolves
    # Atlassian credentials request via ``with_atlassian_creds``
    ...
    worker = Worker(
        ...
        activities=[
            ...
            # Workflow completion dispatch + dept-config lookup activities.
