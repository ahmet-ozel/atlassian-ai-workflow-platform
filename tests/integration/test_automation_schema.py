...
class TestMigrationIdempotence:
 ...
 """,
        ...
        expected_tables = sorted([
            ...
            "processed_events",
            "work_items",
            # Tables added by the foundation migrations.
            "audit_events",
            "probe_artifacts",
        ])
 ...
 once table (no duplicates from repeated CREATE TABLE IF NOT
 EXISTS)."""
        schemas_sql, automation_sql = sql_paths
...
class TestForeignKeyCascadeAndCheckConstraints:
 ...
 The legacy enum {active, shadow, paused, decommissioned} was replaced
 with {active, shadow, disabled} to match departments.schema.json.
 """
