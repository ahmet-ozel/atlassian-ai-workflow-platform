...
""")
        ...
        try:
            # Run the test script. Pin UTF-8 explicitly so Windows'
            # ANSI code page (cp1254 on Turkish installs, cp1252 on
            # English, etc.) cannot corrupt the decoder if the child
            # emits any non-ASCII output. This mirrors the fix applied
            # in ``test_stdio_lifecycle.py``.
