...
# Zero-argument DC read-only tools. These tools wrap Jira DC endpoints
# that have no query parameters (/rest/api/2/myself, /priority, /resolution,
# /status, /issuetype). OpenAI-style gateways that reject zero-arg tools
# must filter them client-side or run through a pass-through adapter —
# adding a dummy parameter here would be dishonest about the tool shape.
_ZERO_ARG_TOOL_EXCEPTIONS: frozenset[str] = frozenset(
    {
        "jira_get_myself",
...
def test_has_parameters(tool_name: str, all_tool_schemas: dict[str, dict]) -> None:
    ...
    except for a small, documented allow-list of pure-lookup DC read
    tools whose wrapped endpoints have no query parameters at all.
    """
    if tool_name in _ZERO_ARG_TOOL_EXCEPTIONS:
        pytest.skip(
            f"{tool_name} wraps a zero-parameter DC endpoint; gateway "
            f"adapters filter these client-side."
        )
...
def test_all_properties_have_type(
    ...
    Exception: tool parameters intentionally typed as ``Any`` to accept
    arbitrary JSON shapes (for example ``confluence_set_page_property.value``
    which stores operator-defined JSON) carry no ``type`` field because the
    Pydantic schema has none to emit. Vertex AI callers that need a type
    here should constrain their callers to a specific JSON shape; we do
    not enforce that here because the DC endpoint itself accepts any JSON.
    """
    _UNTYPED_PROPERTY_EXCEPTIONS: dict[str, frozenset[str]] = {
        "confluence_set_page_property": frozenset({"value"}),
    }
    schema = all_tool_schemas[tool_name]
    properties = schema.get("properties", {})
    exceptions = _UNTYPED_PROPERTY_EXCEPTIONS.get(tool_name, frozenset())
    for prop_name, prop_def in properties.items():
        if not isinstance(prop_def, dict):
            continue
        if prop_name in exceptions:
            continue
