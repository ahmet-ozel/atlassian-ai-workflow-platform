...
4. **EXPOSE matches port mode**: ``EXPOSE <c.container_port>``
...
5. **Healthcheck shape**:
 ...
 project's choice is documented in each Dockerfile's header
...
the project uses (no parser-comments, no `\\` continuations inside
...
(which is listed in ``tests/the operational rule.txt`` as optional). Multi-
 ...
 """
    ...
    while i < len(physical_lines):
        ...
        while body.rstrip().endswith("\\"):
            ...
            # inside a RUN, but the project does not use this).
...
def _assert_multi_stage(
 ...
 """
    ...
    assert len(froms) >= 2, (
        ...
        f"(multi-stage build, the operational rule); found {len(froms)}"
    )
    ...
    assert first_alias == "builder", (
        ...
        f"(the operational rule); got alias={first_match.group('alias')!r}"
    )
    return froms
...
def _assert_base_image(component: ComponentSpec, froms: list[_Directive]) -> None:
    ...
    for from_directive in froms:
        ...
        assert expected_re.match(image), (
            ...
            f"(the operational rule.{'3' if component.runtime == 'python' else '4'})"
        )
 ...
 *before* any ``CMD`` / ``ENTRYPOINT`` directive.
 """
    ...
    assert user_create_line is not None, (
        ...
        f"'adduser -D -u 10001... appuser' (the operational rule)"
    )
    ...
    assert user_directive_line is not None, (
        ...
        f"directive (the operational rule)"
    )
    ...
    for directive in directives:
        if directive.keyword in {"CMD", "ENTRYPOINT"}:
            assert user_directive_line < directive.line_no, (
                ...
                f"{directive.keyword} (line {directive.line_no}); the operational rule"
            )
...
def _assert_expose(
    ...
    if component.type in {"http_service", "ui_component"}:
        ...
        assert port_str in ports_seen, (
            f"{component.name}: missing EXPOSE {port_str} (the operational rule); "
        ...
        assert not expose_directives, (
            ...
            f"EXPOSE directive (the operational rule); got "
...
def _assert_healthcheck(
 ...
 """
    ...
    if component.type == "http_service":
        ...
        # in any flag order; the project uses ``-fsS`` as a single
        ...
        assert re.search(r"\bcurl\b", cmd), (
            ...
            f"curl (the operational rule); got {cmd!r}"
        )
        assert re.search(r"-fsS\b|-fSs\b|-Sfs\b|-sfS\b|-fs\b.*-S\b|-f\b.*-s\b.*-S\b", cmd), (
            ...
            f"'-fsS' flags (the operational rule); got {cmd!r}"
        )
        assert re.search(url_pattern, cmd), (
            f"{component.name}: HEALTHCHECK URL must be "
            f"http://localhost:{port}/healthz (the operational rule); got {cmd!r}"
        )
        ...
        assert re.search(r"\bpython\b\s+-c\b", cmd), (
            ...
            f"'python -c' one-liner (the operational rule); got {cmd!r}"
        )
        assert re.search(r"\btemporalio\.client\b", cmd) or re.search(
            ...
            f"temporalio.client.Client class (the operational rule); got {cmd!r}"
        )
        assert re.search(r"\bClient\.connect\b", cmd), (
            f"{component.name}: worker HEALTHCHECK must call "
            f"Client.connect(...) (the operational rule); got {cmd!r}"
        )
        ...
        assert re.search(r"\b(curl|wget)\b", cmd), (
            ...
            f"(the operational rule); got {cmd!r}"
        )
        port = component.container_port
        assert port is not None, (
        ...
        assert re.search(rf"http://localhost:{port}/", cmd), (
            ...
            f"http://localhost:{port}/... (the operational rule); got {cmd!r}"
        )
    else:
        raise AssertionError(
