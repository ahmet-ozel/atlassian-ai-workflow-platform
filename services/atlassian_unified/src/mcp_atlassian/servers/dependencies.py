...
from mcp_atlassian.utils.environment import _detect_bitbucket_auth_from_headers
...
class _ServiceSpec:
    ...
    username_header: str | None = None  # X-Atlassian-{Service}-Username
    api_token_header: str | None = None  # X-Atlassian-{Service}-Api-Token
    # --- Bitbucket-only Cloud credential header slots (Requirement 3.6-3.9,
    # 17.1-17.3). Jira/Confluence leave these as ``None``; the Bitbucket
    # spec populates them with the canonical capitalisation used by the
    # middleware so the per-request header branch can resolve Cloud auth
    # via ``_detect_bitbucket_auth_from_headers``. Adding them as optional
    # fields on the shared ``_ServiceSpec`` keeps the generic
    # ``_get_fetcher`` dispatcher single-table: Jira/Confluence simply
    # never read them.
    cloud_access_token_header: str | None = None
    cloud_app_password_header: str | None = None
    cloud_username_header: str | None = None
...
def _jira_spec() -> _ServiceSpec:
    ...
    return _ServiceSpec(
        ...
        username_header="X-Atlassian-Jira-Username",
        api_token_header="X-Atlassian-Jira-Api-Token",  # noqa: S106
...
def _confluence_spec() -> _ServiceSpec:
    ...
    return _ServiceSpec(
        ...
        username_header="X-Atlassian-Confluence-Username",
        api_token_header="X-Atlassian-Confluence-Api-Token",  # noqa: S106
...
def _create_bitbucket_user_config(
    base_config: BitbucketConfig,
    auth_type: str,
    credentials: dict[str, Any],
) -> BitbucketConfig:
    """Create a user-specific BitbucketConfig.

    Bitbucket Server/Data Center has a different schema from Jira/Confluence:
    no ``api_token``/``oauth_config`` fields, and Basic auth uses
    ``username``/``password`` rather than email/API token. OAuth Bearer tokens
    are resolved upstream to PAT via ``_resolve_bearer_auth_type`` because
    Bitbucket DC does not expose an OAuth flow on its REST API surface here.
    """
    if auth_type == "pat":
        user_pat = credentials.get("personal_access_token")
        if not user_pat:
            raise ValueError(
                "PAT missing in credentials for Bitbucket auth_type 'pat'"
            )
    ...
    if auth_type == "basic":
        # Middleware extracts ``Authorization: Basic`` as user_email/api_token;
        # for Bitbucket DC these are the username/password pair.
        username = credentials.get("user_email")
        password = credentials.get("api_token")
        if not username or not password:
            raise ValueError(
                "Username and password missing in credentials for Bitbucket "
                "auth_type 'basic'"
            )
        return dataclasses.replace(
            base_config,
    ...
    raise ValueError(
        "OAuth authentication is not supported by Bitbucket Server/Data Center. "
        "Use a PAT (Authorization: Token <PAT>) or Basic auth instead."
    )
...
def _create_user_config_for_fetcher(
    ...
    """Create a user-specific configuration for Jira/Confluence/Bitbucket fetchers.
    ...
    Returns:
        Service-specific config with user credentials applied.
    ...
    # Bitbucket DC has a different schema (no api_token/oauth_config fields,
    # Basic auth uses username/password) — handle it in a dedicated helper to
    # avoid TypeError on dataclasses.replace.
    if isinstance(base_config, BitbucketConfig):
        return _create_bitbucket_user_config(base_config, auth_type, credentials)
...
async def _get_fetcher(ctx: Context, spec: _ServiceSpec) -> Any:
    """Generic fetcher resolution for Jira, Confluence, and Bitbucket.

    Handles header-based PAT, basic auth, OAuth/PAT, and global fallback.
    For Bitbucket DC, OAuth Bearer tokens are downgraded to PAT in
    ``_resolve_bearer_auth_type`` since Bitbucket DC has no OAuth flow.
    """
    fn_name = f"get_{spec.name.lower()}_fetcher"
    ...
    try:
        ...
        username_header_val = (
            service_headers.get(spec.username_header)
            if spec.username_header is not None
            else None
        )
        api_token_header_val = (
            service_headers.get(spec.api_token_header)
            if spec.api_token_header is not None
            else None
        )

        # --- Branch 0: Bitbucket-only Cloud credential header resolution ---
        # Rows B, C, D of the Bitbucket auth truth table (design Section
        # "Authentication: truth table"). Only the Bitbucket spec registers
        # Cloud credential header slots, so Jira/Confluence skip this
        # branch entirely (Requirements 3.6, 3.7, 3.8, 3.9, 17.1-17.5).
        if spec.cloud_access_token_header is not None:
            header_auth = _detect_bitbucket_auth_from_headers(service_headers)

            # Row D: CloudHost URL header present without any Cloud
            # credential header. Reject the request with HTTP 401 before
            # any outbound Bitbucket call (Requirement 17.4). The existing
            # tool-layer error handling converts
            # ``MCPAtlassianAuthenticationError`` into the structured 401
            # response the HTTP adapter emits.
            if header_auth.unauthorized:
                logger.info(
                    f"{fn_name}: Rejecting Bitbucket request — Cloud URL "
                    f"{header_auth.url!r} present without any Cloud "
                    "credential header (row D). Returning 401 without "
                    "outbound Bitbucket call."
                )
                raise MCPAtlassianAuthenticationError(
                    "Unauthorized: Bitbucket Cloud URL header present "
                    "without any Cloud credential header "
                    "(X-Atlassian-Bitbucket-Cloud-Access-Token or "
                    "X-Atlassian-Bitbucket-Username + "
                    "X-Atlassian-Bitbucket-App-Password)."
                )

            # Row B: Cloud OAuth 2.0 bearer token. Priority over Basic per
            # the truth table.
            if (
                header_auth.auth_type == "cloud_bearer"
                and header_auth.url
                and header_auth.cloud_access_token
                and not hasattr(request.state, "user_atlassian_token")
            ):
                logger.info(
                    f"Creating header-based {spec.name}Fetcher "
                    f"with URL: {header_auth.url} and Cloud bearer token"
                )
                header_config = spec.config_class(
                    url=header_auth.url,
                    auth_type="cloud_bearer",
                    cloud_access_token=header_auth.cloud_access_token,
                    ssl_verify=True,
                    http_proxy=None,
                    https_proxy=None,
                    no_proxy=None,
            ...
            # Row C: Cloud Basic with App Password. Both username and
            # app-password headers must be present.
            if (
                header_auth.auth_type == "basic"
                and header_auth.is_cloud
                and header_auth.url
                and header_auth.username
                and header_auth.app_password
                and not hasattr(request.state, "user_atlassian_token")
            ):
                logger.info(
                    f"Creating header-based {spec.name}Fetcher "
                    f"with URL: {header_auth.url} and Cloud app password"
                )
                header_config = spec.config_class(
                    url=header_auth.url,
                    auth_type="basic",
                    username=header_auth.username,
                    app_password=header_auth.app_password,
                    ssl_verify=True,
                    http_proxy=None,
                    https_proxy=None,
                    no_proxy=None,
        ...
        # --- Branch 1: header-based Cloud Basic/API token ---
        if (
            url_header_val
            and username_header_val
            and api_token_header_val
            and not hasattr(request.state, "user_atlassian_token")
        ):
            logger.info(
                f"Creating header-based {spec.name}Fetcher "
                f"with URL: {url_header_val} and API token"
            )
            header_config = spec.config_class(
                url=url_header_val,
                auth_type="basic",
...
def _bitbucket_on_validated(
    ...
    """Post-validation logging for Bitbucket."""
    if auth_branch == "header_pat":
        logger.debug(
            f"{fn_name}: Validated header-based Bitbucket token"
        )
    else:
        logger.debug(f"{fn_name}: Validated Bitbucket token")


def _bitbucket_probe(fetcher: Any) -> Any:
    """Run the Bitbucket connectivity probe appropriate for the effective mode.

    DC mode (``BitbucketFetcher.is_cloud`` is ``False``) uses the existing
    ``get_projects(limit=1)`` probe byte-for-byte so the DC
    credential-validation path does not change (Requirements 18.1, 18.4).

    Cloud mode originally issued ``GET /2.0/workspaces?pagelen=1`` per
    Requirement 18.2, but Atlassian removed that endpoint in CHANGE-2770
    (September 2025) and it now returns HTTP 410 Gone. The probe is
    therefore updated to use ``GET /2.0/user`` which is the documented
    Cloud endpoint for "who am I" validation. If that endpoint is rejected
    (403 "This API is not accessible by this authentication mechanism"
    for workspace / repository access tokens whose scope does not include
    ``account``), we fall back to a workspace-scoped probe against
    ``/2.0/workspaces/{workspace}`` when a workspace is configured, and
    finally accept a "no-op success" so startup is not blocked by the
    API deprecation for operators who rely on per-request auth (no
    server-side credentials at lifespan time).

    Returns:
        A truthy response object on success. Non-2xx responses are mapped
        onto ``MCPAtlassianAuthenticationError`` / ``HTTPError`` by the
        session layer and turned into a ``ValueError`` wrapper by
        ``_create_and_validate`` — that behavior is preserved.
    """
    if not fetcher.is_cloud:
        return fetcher.get_projects(limit=1)

    # Cloud: try /2.0/user first. This is the lightest possible auth probe
    # that replaces the now-removed /2.0/workspaces listing.
    try:
        return fetcher.bitbucket.get("/2.0/user")
    except Exception as user_err:  # noqa: BLE001 — fall through to workspace probe
        logger.debug(
            "Bitbucket Cloud /2.0/user probe failed (%s); "
            "falling back to workspace-scoped probe.",
            user_err,
        )

    # Fallback 1: workspace-scoped probe when a workspace is configured.
    workspace = getattr(fetcher.config, "workspace", None)
    if workspace:
        return fetcher.bitbucket.get(f"/2.0/workspaces/{workspace}")

    # Fallback 2: no workspace configured and /2.0/user denied. Return a
    # minimal success object so lifespan does not crash; per-request calls
    # will surface their own auth errors as they happen. This is the same
    # posture as an empty DC project list — the probe proves only
    # reachability, not feature-by-feature authorization.
    logger.info(
        "Bitbucket Cloud connectivity probe could not validate credentials "
        "via /2.0/user and no BITBUCKET_WORKSPACE is configured — "
        "treating probe as a no-op. Per-request tool calls will validate "
        "their own credentials."
    )
    return {"values": [], "probe": "skipped"}


def _bitbucket_spec() -> _ServiceSpec:
    """Build Bitbucket service spec.

    Deferred to a function so test patches are picked up at call time.
    """
    return _ServiceSpec(
        name="Bitbucket",
        ...
        filter_kwargs={"projects_filter": None},
        get_session=lambda f: f.bitbucket._session,
        validate_fn=_bitbucket_probe,
        on_validated=_bitbucket_on_validated,
        # Cloud credential header slots (Requirements 3.6, 3.7, 3.8, 3.9,
        # 17.1, 17.2, 17.3). Jira/Confluence leave these at ``None``.
        cloud_access_token_header="X-Atlassian-Bitbucket-Cloud-Access-Token",  # noqa: S106
        cloud_app_password_header="X-Atlassian-Bitbucket-App-Password",  # noqa: S106
        cloud_username_header="X-Atlassian-Bitbucket-Username",
    )
