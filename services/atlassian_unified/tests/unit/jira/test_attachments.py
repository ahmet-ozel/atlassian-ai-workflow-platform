...
class TestAttachmentsMixin:
    ...
    def test_download_attachment_success(self, attachments_mixin: AttachmentsMixin):
        ...
        # Use a path that ``os.path.isabs`` reports as absolute on every
        # platform so the code under test skips ``abspath`` and the literal
        # string we pass shows up in the ``open()`` call.
        ...
        with (
            ...
            mock_file.assert_called_once_with(download_path, "wb")
