...
def pytest_addoption(parser: pytest.Parser) -> None:
    """Add --dc-e2e, --cloud-e2e, and --bitbucket-cloud-e2e command-line options."""
    parser.addoption(
        "--dc-e2e",
        action="store_true",
        default=False,
    ...
    parser.addoption(
        ...
        action="store_true",
        default=False,
        help="Run E2E tests against a live Bitbucket Cloud tenant",
    )


def pytest_configure(config: pytest.Config) -> None:
    """Register dc_e2e, cloud_e2e, and bitbucket_cloud_e2e markers."""
    ...
    config.addinivalue_line(
        "markers",
        "bitbucket_cloud_e2e: marks tests that require a live Bitbucket Cloud tenant",
    )


def _get_missing_bitbucket_cloud_env_vars() -> list[str]:
    """Return names of required env vars missing for bitbucket_cloud_e2e tests.
    ...
    if not os.environ.get("BITBUCKET_CLOUD_URL") and not os.environ.get(
        "BITBUCKET_URL"
    ):
        missing.append("BITBUCKET_CLOUD_URL")
...
def pytest_collection_modifyitems(
    ...
    """Auto-skip dc_e2e/cloud_e2e/bitbucket_cloud_e2e tests unless their flags are passed."""
    run_dc = config.getoption("--dc-e2e")
    run_cloud = config.getoption("--cloud-e2e")
    run_bb_cloud = config.getoption("--bitbucket-cloud-e2e")

    skip_dc = pytest.mark.skip(reason="need --dc-e2e option to run")
    skip_cloud = pytest.mark.skip(reason="need --cloud-e2e option to run")
    skip_bb_cloud = pytest.mark.skip(reason="need --bitbucket-cloud-e2e option to run")

    # Env-gating for bitbucket_cloud_e2e: check once, apply to all marked tests
    bb_cloud_missing_vars = _get_missing_bitbucket_cloud_env_vars()
    ...
    for item in items:
        ...
        if "bitbucket_cloud_e2e" in item.keywords:
            if not run_bb_cloud:
                item.add_marker(skip_bb_cloud)
            elif bb_cloud_missing_vars:
                reason = (
                    "Skipping bitbucket_cloud_e2e: missing "
                    + ", ".join(bb_cloud_missing_vars)
                )
