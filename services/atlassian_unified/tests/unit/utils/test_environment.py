...
def _assert_service_availability(
    ...
    Bitbucket was added later than Jira/Confluence; the existing test
    scenarios don't exercise it, so the helper defaults its expectation to
    ``False`` to keep call sites concise while still covering the new key.
    """
    assert result == {
