"""Property 20 — Compose bootstrap minimal footprint.

**Validates: Requirements 7.1** (platform-gap-fill spec)
...
behind ``--profile <name>``. Property 20 therefore reduces to a
...
* Hypothesis explores **mutations** of the parsed document — for each
  ...
  fictional profile-less service. Property 20 says any of these
...
# ---------------------------------------------------------------------------
# Boot_Bundle definition — platform-gap-fill Requirement 7.1
# ---------------------------------------------------------------------------

#: Exact set of services that ``docker compose -f
#: infra/docker-compose.yml up -d`` (no ``--profile`` flag) MUST start
#: per platform-gap-fill Requirement 7.1 / Property 20. The set is
...
def _load_manifest() -> dict[str, Any] | None:
    ...
    fields like ``smoke_test_command``) so this property test stays
    decoupled from upstream schema-evolution work. Returns ``None`` if
    the manifest is absent — the manifest is optional for Property 20
...
def _profiles_of(service: dict[str, Any]) -> list[str] | None:
    ...
    * **A non-empty list of strings** — the service is *only* included
    ...
    present but empty" in error messages — Property 20 collapses both
...
# ---------------------------------------------------------------------------
# Property 20 (canonical) — Boot_Bundle equals the default-profile set
# ---------------------------------------------------------------------------


def test_compose_default_profile_set_equals_boot_bundle() -> None:
    """Property 20 — exactly the Boot_Bundle is in the default-profile set.
    ...
    assert not missing, (
        ...
        f"the default-profile set (Requirement 7.1, Property 20); "
        f"missing={sorted(missing)!r}; "
        f"actual default-profile set={sorted(actual)!r}"
    )
    assert not extra, (
        "infra/docker-compose.yml: services present in the default-"
        "profile set that are NOT in the Boot_Bundle (Requirement 7.1, "
        "Property 20). A bare 'docker compose up -d' would start "
    ...
    assert actual == BOOT_BUNDLE, (
        ...
        f"Boot_Bundle (Requirement 7.1, Property 20); "
...
def test_boot_bundle_anchor_default_profile_set(boot_service: str) -> None:
    ...
    assert service is not None, (
        ...
        f"infra/docker-compose.yml (Requirement 7.1, Property 20); "
    ...
    assert profiles is None or profiles == [], (
        ...
        f"(Requirement 7.1, Property 20); got profiles={profiles!r}"
    )
...
def test_non_boot_bundle_services_are_profile_gated(service_name: str) -> None:
    ...
    ``docker compose up -d``. This is the symmetric half of
    :func:`test_compose_default_profile_set_equals_boot_bundle` and
...
def test_manifest_non_boot_bundle_compose_profile_is_non_empty_string(
    ...
    """Property 20 (manifest side) — non-Boot_Bundle entries declare a compose_profile.

    Validates: Requirement 7.1, R7.3 (manifest is the source of truth
    ...
    assert isinstance(profile, str), (
        ...
        f"for non-Boot_Bundle services (Requirement 7.1/7.3, Property "
...
def _candidate_with_extra_default_service(extra_name: str) -> dict[str, dict[str, Any]]:
    ...
    service to the stack". Property 20 says this MUST take the
...
def _candidate_with_demoted_boot_bundle(demoted: str) -> dict[str, dict[str, Any]]:
    ...
    drops out of the default-profile set. Property 20 says this MUST
...
def _candidate_with_promoted_profile_service(
    ...
    enters the default-profile set. Property 20 says this MUST leave
...
def test_property20_mutations_violate_boot_bundle_equality(
    ...
    """Property 20 (mutation) — any structural mutation breaks the equality.
    ...
    deterministically). This codifies the contract "Property 20 is
    ...
    * ``promote_gated`` — drops the ``profiles:`` directive from a
    ...
    # Sanity — the canonical document MUST satisfy Property 20 for
    ...
    assert canonical == BOOT_BUNDLE, (
        f"canonical compose document violates Property 20 "
    ...
    if kind == "add_extra":
        ...
        assert candidate_set != BOOT_BUNDLE, (
            f"Property 20 mutation 'add_extra({extra_name!r})' did NOT "
    ...
    if kind == "demote_boot":
        ...
        assert candidate_set != BOOT_BUNDLE, (
            f"Property 20 mutation 'demote_boot({target!r})' did NOT "
            f"break the Boot_Bundle equality; got "
            f"candidate_set={sorted(candidate_set)!r}, "
            f"BOOT_BUNDLE={sorted(BOOT_BUNDLE)!r}"
        )
    ...
    assert candidate_set != BOOT_BUNDLE, (
        f"Property 20 mutation 'promote_gated({target!r})' did NOT "
        f"break the Boot_Bundle equality; got "
        f"candidate_set={sorted(candidate_set)!r}, "
        f"BOOT_BUNDLE={sorted(BOOT_BUNDLE)!r}"
    )
