"""Property test 7 — Cost prediction + budget cap enforcement.

**Validates: Requirements 5.5, 5.6, 5.7**

Hypothesis-driven verification of the cost-prediction fallback rule
and the budget-cap enforcement state machine described in
``design.md`` §"CostPredictor (saf fonksiyon)" and
§"BudgetCapPolicy (\\``automation-service/src/budget/policy.py``)``,
implementing tasks.md §7.6 of ``platform-mimari-ops``.
...
* :class:`automation_service.budget.policy.BudgetCapPolicy` (task
  7.3 — already shipped) is exercised end-to-end with an
  in-memory :class:`UsageQueryRunner` and
  :class:`AuditWriter` so the property test owns the full
  decide-and-audit contract.
* :func:`cost_tracking.predictor.predict_cost` (task 7.2) is
  ...
  implementation must satisfy. When task 7.2 ships, the import
...
* ``platform-mimari-ops/design.md`` §"Property 7" — invariant set.
* ``platform-mimari-ops/design.md`` §"Sequence: Cost prediction +
  budget cap enforcement" — the audit-on-fallback callsite the
  Property reaches through.
* ``platform/services/automation-service/tests/unit/\
  test_budget_policy.py`` — example-based tests that pin
  individual scope branches; this property test owns the
...
# so the property test runs identically from any cwd.
# ---------------------------------------------------------------------------
...
# defensively for the day task 7.2 ships its production module.
...
# task 7.2; until it ships we fall back to an in-test stand-in that
# mirrors the design pseudocode so clauses (a) / (b) still pin the
# contract a future implementation must satisfy.
# ---------------------------------------------------------------------------

try:  # pragma: no cover - guard collapses once task 7.2 ships
    ...
    _PRODUCTION_MIN_TASKS = 30  # design pseudocode default


#: The fallback threshold from ``design.md`` §"CostPredictor".
...
#
# These mirror the shape declared in ``design.md`` §"Cost & Budget
# Dataclass'ları". Once task 7.2 lands we replace the local
...
class _CostPrediction:
    """Mirror of ``design.md`` :class:`CostPrediction` data model.

    The four-attribute shape is the contract the
    ``automation-service`` workflow-start handler relies on; clauses
    (a) and (b) of Property 7 pin the relationship between the
...
class _BudgetUsageInput:
    ...
    The property test populates four scope-specific aggregates and
...
# ---------------------------------------------------------------------------
# Reference predictor — fall-back implementation used when task 7.2
...
def _predict_cost(**kwargs: Any) -> _CostPrediction:
    ...
    Once task 7.2 lands the production callable replaces the
...
class _FakeUsageRunner:
    ...
    issues so clause (e) of Property 7 — the ``cost_tag =
...
def _expected_decision(
    *,
    ...
    """Compute the decision the policy MUST return per Property 7 (c).

    Separately re-implementing the four-scope ladder gives us an
    oracle to compare against the policy output without coupling
    the test to internal helpers. The ordering must match
    :data:`SCOPE_ORDER` exactly.
    """
...
def _budget_caps_strategy(draw: st.DrawFn) -> BudgetCaps:
    ...
    The caps must be ``> 0`` so the ``>=`` comparison in
    :meth:`BudgetCapPolicy.enforce` has a meaningful boundary. We
...
def _budget_usage_strategy(draw: st.DrawFn) -> _BudgetUsageInput:
    ...
    that breach at least one cap from
    :func:`_budget_caps_strategy`. Property 7 (c) requires us to
...
# ---------------------------------------------------------------------------
# Property 7 — predictor source switch + CI invariant
# ---------------------------------------------------------------------------
...
def test_predict_cost_source_and_ci_invariant(
    ...
    """Property 7 (a) — source switch on ``task_count >= 30`` + CI bounds.
    ...
    * ``confidence_low <= predicted_usd <= confidence_high`` for
    ...
    if dept.task_count >= GLOBAL_FALLBACK_MIN_TASKS:
        assert pred.source == "dept", (
            ...
            "Property 7 (a) requires ``source='dept'`` once the dept "
        ...
        assert pred.source == "global_fallback", (
            f"task_count={dept.task_count} < "
            f"{GLOBAL_FALLBACK_MIN_TASKS} but source={pred.source!r}; "
            "Property 7 (a) requires the global cross-dept fallback "
    ...
    assert pred.confidence_low <= pred.predicted_usd <= pred.confidence_high, (
        ...
        f"high={pred.confidence_high}. Property 7 (a) requires "
...
def test_predict_cost_is_deterministic(
    ...
    """Property 7 (f) — predictor is a pure function.
...
# ---------------------------------------------------------------------------
# Property 7 — BudgetCapPolicy enforcement state machine
# ---------------------------------------------------------------------------
...
def test_budget_cap_policy_decision_table(
    ...
    """Property 7 (c) + (d) — scope-ordered deny + single audit on first breach.
    ...
    * On allow no audit event is written; on deny **exactly one**
    ...
    assert decision == expected, (
        ...
        "Property 7 (c) requires the deterministic order "
        f"{SCOPE_ORDER!r}."
    )
    ...
    if decision.allowed:
        assert writer.events == [], (
            ...
            "decision; Property 7 (d) forbids any audit traffic "
    ...
    assert len(writer.events) == 1, (
        ...
        f"events={writer.events!r}. Property 7 (d)."
    )
    event = writer.events[0]
    assert event.action == "budget_exceeded", (
        f"Deny audit action={event.action!r}; Property 7 (d) "
    ...
    assert payload.get("scope") == decision.deny_scope, (
        ...
        "Property 7 (d) ties the two together."
    )
...
def test_budget_cap_policy_excludes_non_production_via_sql_filter(
    ...
    """Property 7 (e) — the policy SQL filters ``cost_tag='production'``.
    ...
    for query, _args in runner.calls:
        assert "cost_tag = 'production'" in query, (
            ...
            f"{query!r}. Property 7 (e) requires sandbox / probe "
...
def test_budget_cap_policy_is_deterministic(
    ...
    """Property 7 (f) — repeating the call is deterministic.
    ...
    assert decision_a == decision_b, (
        ...
        f"{decision_a!r} vs {decision_b!r}. Property 7 (f)."
    )
...
# ---------------------------------------------------------------------------
# Property 7 — concrete regression anchors
# ---------------------------------------------------------------------------
...
def test_dept_weekly_breach_denies_first_in_scope_order() -> None:
    ...
    Property 7 (c) — scope-ordered evaluation. Both ``dept_weekly``
...
def test_user_id_none_skips_user_scoped_caps() -> None:
    ...
    Property 7 (c) — when ``user_id is None`` the user-scoped checks
...
def test_predict_cost_threshold_boundary() -> None:
    ...
    Property 7 (a) uses ``>=`` so the boundary value belongs to
...
def test_predict_cost_below_threshold_uses_global_fallback() -> None:
    ...
    Property 7 (a). Hitting the fallback is mandatory for the
    cold-start path that ``audit cost_prediction_using_global_fallback``
    depends on (Requirement 5.7).
