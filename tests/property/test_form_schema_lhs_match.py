"""Property P4 — Form schema = Component_Env_Example LHS key set.
...
preserved (Requirement 5.1: "form, dosyada görünen sırada üretilir";
Requirement 5.2: "yorum metni input alanının yardım metnine bağlanır";
Requirement 5.6: form_schema LHS = .env.example LHS as exact set
equality).

This is the property-level proof that the *data path* between
``parse_env_example`` (the pure parser, design §4.7) and
``LifecycleService.get_form_schema`` (the form-rendering surface,
design §3.3) cannot drift: every operator-facing form field
...
------------------------------
Requirement 5.2 / design §4.7 also obliges the parser to be
deterministic across "comment + blank line + assignment" combinations.
...
/ ``_FakeHealthProbe`` shells used by Property C5
...
assert _MANAGED_SERVICES, (
    ...
    "for Property P4 to be meaningful"
)
...
class _FakeComposeRunner:
    ...
    async def up(  # pragma: no cover - unused by Property P4
        self,
        *,
        profile: str,
        service_name: str,
    ...
    async def stop(  # pragma: no cover - unused by Property P4
    ...
    async def logs(  # pragma: no cover - unused by Property P4
    ...
    async def exec_test(  # pragma: no cover - unused by Property P4
        self,
        *,
        service_name: str,
        argv: Sequence[str],
...
class _FakeHealthProbe:
    ...
    async def probe(  # pragma: no cover - unused by Property P4
...
# line classifier (Rule 2 with body, Rule 2 without body, Rule 3).
...
# ---------------------------------------------------------------------------
# Property P4 — form schema LHS set == .env.example LHS set
# ---------------------------------------------------------------------------
...
def test_form_schema_keys_equal_env_example_lhs_keys(
    ...
    """Property P4 (core invariant) — form schema LHS set == .env.example LHS set.
    ...
    missing or extra keys — the set of ``key`` values returned by
    :meth:`LifecycleService.get_form_schema(entry.name)`. Per
    Requirement 5.1, the ordering returned by ``get_form_schema``
    ...
    # Invariant 1 — exact set equality (Requirement 5.6).
    ...
    # Invariant 2 — file-order preservation (Requirement 5.1).
    ...
    # This isn't required by Requirement 5.6 alone, but Requirement
    # 5.2 ("yorum metni input alanının yardım metnine bağlanır")
    # depends on ``comment`` round-tripping through the schema
    # without modification, and Requirement 5.3 (Sensitive_Env_Key
    # masking) depends on ``is_sensitive`` round-tripping too. Both
    # are folded into Property P4 here so a regression in either
    # direction fails this single property test.
...
# ---------------------------------------------------------------------------
# Property P4 — parser determinism under comment + blank-line perturbation
# ---------------------------------------------------------------------------
...
def test_parser_is_deterministic_under_comment_and_blank_line_perturbations(
    ...
    """Property P4 (parser determinism) — comments / blanks don't change LHS set.
    ...
    This is the design §4.7 contract that comment lines feed the
    comment buffer and blank lines reset it, but neither line shape
    can spawn or suppress an assignment-derived field.
    """
...
def test_each_manifest_service_yields_a_non_empty_form_schema(
    ...
    construction of the scaffold — see scaffold spec §10).
    """
