...
from mcp_atlassian.utils.dc_guards import DCVersionProbe
...
class JiraClient(DCVersionProbe):
    ...
    def get_dc_version(self) -> str | None:
        """Lazily probe and cache the Jira Data Center version.

        On first call this issues ``GET /rest/api/2/serverInfo`` against the
        configured Jira instance and extracts the ``version`` field from the
        JSON response (a dict shaped like
        ``{"version": "9.4.0", "versionNumbers": [9, 4, 0],
        "deploymentType": "Server", ...}``). The resulting string is cached
        on ``self._dc_version`` so subsequent invocations and every
        downstream ``check_dc_version`` call reuse the same probe without
        additional HTTP traffic.

        Any failure during the probe (network error, unexpected payload
        shape, missing ``version`` field) leaves ``self._dc_version`` as
        ``None`` so :func:`mcp_atlassian.utils.dc_guards.check_dc_version`
        can fall through to ``dc_version_unknown`` once a tool attempts the
        actual call and sees a 404 / 501.
        ...
        Returns:
            The detected version string (for example ``"9.4.0"``) or
        ...
        try:
            response = self.jira.get("rest/api/2/serverInfo")
        except Exception as e:  # noqa: BLE001 — probe must never raise
            logger.debug(
                f"Jira DC version probe failed against "
                f"/rest/api/2/serverInfo: {e}"
            )
            return None

        if not isinstance(response, dict):
            logger.debug(
                f"Jira DC version probe returned unexpected payload shape: "
        ...
        if not isinstance(version, str) or not version.strip():
            logger.debug(
                "Jira DC version probe response did not contain a "
                "usable 'version' field; caching None."
            )
            return None

        self._dc_version = version
        logger.debug(f"Detected Jira DC version: {version}")
        return self._dc_version
