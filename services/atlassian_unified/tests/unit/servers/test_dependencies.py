...
class TestBitbucketUserConfigCreation:
    """Tests for _create_user_config_for_fetcher with BitbucketConfig.

    Bitbucket DC has a different schema than Jira/Confluence (no api_token /
    oauth_config, Basic auth uses username/password) so it needs its own
    dispatch in _create_user_config_for_fetcher.
    """

    @staticmethod
    def _base() -> "BitbucketConfig":
        from mcp_atlassian.bitbucket import BitbucketConfig

        return BitbucketConfig(
            url="https://bb.example.com",
            auth_type="pat",
            personal_token="base-pat",
            projects_filter="ENG",
        )

    def test_pat_auth_via_authorization_token_header(self):
        """Authorization: Token <PAT> path should produce a per-user PAT config."""
        from mcp_atlassian.bitbucket import BitbucketConfig
        ...
        result = _create_user_config_for_fetcher(
            ...
            auth_type="pat",
            credentials={
                "personal_access_token": "user-pat",
        ...
        assert result.personal_token == "user-pat"
        assert result.username is None
        assert result.password is None
        assert result.url == "https://bb.example.com"
        assert result.projects_filter == "ENG"

    def test_basic_auth_maps_to_username_and_password(self):
        """Authorization: Basic must map email/api_token onto username/password."""
        from mcp_atlassian.bitbucket import BitbucketConfig
        ...
        assert result.password == "s3cret-pw"
        assert result.personal_token is None
        assert result.projects_filter == "ENG"

    def test_oauth_auth_type_is_rejected(self):
        """Bitbucket DC does not support OAuth — must raise ValueError."""
        with pytest.raises(ValueError, match="OAuth.*not supported"):
            _create_user_config_for_fetcher(
                base_config=self._base(),
                auth_type="oauth",
                credentials={"oauth_access_token": "ignored"},
            )

    def test_pat_auth_missing_token_raises(self):
        """An empty PAT must fail loudly rather than silently producing junk."""
    ...
    def test_basic_auth_missing_credentials_raises(self):
        """Basic auth without both username and password must error."""
        with pytest.raises(ValueError, match="Username and password missing"):
            _create_user_config_for_fetcher(
                base_config=self._base(),
                auth_type="basic",
                credentials={"user_email": "jdoe"},
            )

    def test_per_request_isolation(self):
        """Two consecutive calls must not bleed state between users."""
        from mcp_atlassian.bitbucket import BitbucketConfig

        base = self._base()

        user_a = _create_user_config_for_fetcher(
            base_config=base,
            auth_type="pat",
            credentials={"personal_access_token": "user-a-pat"},
        )
        user_b = _create_user_config_for_fetcher(
            base_config=base,
            auth_type="basic",
            credentials={"user_email": "userb", "api_token": "userb-pw"},
        )

        assert isinstance(user_a, BitbucketConfig)
        assert isinstance(user_b, BitbucketConfig)
        assert user_a is not user_b
        assert user_a.auth_type == "pat" and user_a.personal_token == "user-a-pat"
        assert user_b.auth_type == "basic" and user_b.username == "userb"
        # Base config must remain unchanged.
        assert base.personal_token == "base-pat"
        assert base.username is None and base.password is None
