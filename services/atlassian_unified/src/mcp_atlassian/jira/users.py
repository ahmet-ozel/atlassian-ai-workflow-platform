...
class UsersMixin(JiraClient):
    ...
    def get_current_user_profile(self) -> dict[str, Any]:
        """Return the authenticated user's full profile (``/myself``).

        Works on both Cloud and Server/Data Center; the field set differs
        slightly (Cloud exposes ``accountId`` while Server/DC uses ``key``
        and ``name``) — callers should normalise as needed.

        Returns:
            Raw user dictionary as returned by Jira.
        """
        try:
            data = self.jira.myself()
        except HTTPError as http_err:
            logger.error(f"Failed to fetch current user profile: {http_err}")
            raise Exception(
                f"Unable to fetch current user profile: {http_err}"
            ) from http_err
    ...
    def search_users(
        ...
        """Search for Jira users by name/displayName/email substring.

        On Cloud this uses ``/rest/api/2/user/search?query=`` with rich
        substring matching; on Server/DC the same endpoint accepts the
        ``username`` parameter (case-insensitive substring match).

        Args:
            query: Substring to match. Empty/None on DC returns all users
                up to ``max_results``; on Cloud an empty query returns no
                results.
            max_results: Maximum number of users to return.

        Returns:
            List of ``JiraUser`` models.
        """
        params: dict[str, Any] = {"maxResults": max_results}
        if self.config.is_cloud:
            params["query"] = query or ""
        else:
            # Server/DC parameter name is ``username``
            params["username"] = query or "."

        try:
            response = self.jira.get("rest/api/2/user/search", params=params)
        except HTTPError:
            raise

        if not isinstance(response, list):
            return []
        return [JiraUser.from_api_response(u) for u in response]
    ...
    def search_assignable_users(
        ...
        """Search users that can be assigned issues in a given project.

        Uses ``/rest/api/2/user/assignable/search``. Useful before calling
        ``update_issue`` to set an assignee.

        Args:
            project_key: Project key the user must be assignable in.
            query: Cloud uses ``query`` (substring), Server/DC uses
                ``username``. Empty/None permitted on DC.
            max_results: Maximum number of users to return.

        Returns:
            List of ``JiraUser`` models eligible to be assigned issues.
        """
        params: dict[str, Any] = {
            "project": project_key,
        ...
        try:
            response = self.jira.get(
                "rest/api/2/user/assignable/search", params=params
            )
        except HTTPError:
            raise
