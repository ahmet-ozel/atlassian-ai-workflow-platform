...
3. :func:`automation_worker.workflows.bot_branch_retention.should_delete_branch`
   ...
   (task 2.4 / R10.2).
4. The pure idempotency guards inside
   :func:`agent_runner.activities.iter_advance.iter_advance_pr_supersede`
   — :func:`_build_banner` is a deterministic formatter and
   :func:`_description_already_banners` is a pure idempotency check
   that prevents a doubly-prefixed description on retry (task 7.6 /
   R10.1).

**Validates: Requirements 10.1, 10.2, 10.3, 10.4**

Property statements (design.md §"Property 8")
---------------------------------------------

* **PR supersede idempotency (R10.1)** — re-running the iter-advance
...
* **Branch retention predicate (R10.2)** —
  :func:`should_delete_branch` returns ``True`` iff the branch age
...
* **Orphan-branch filter (R10.3)** — :func:`compute_orphan_branches`
...
* **PO Review Inbox filter (R10.4)** — :func:`compute_po_review_inbox`
...
# property suite finishes inside the per-property deadline.  The
...
# ---------------------------------------------------------------------------
# compute_orphan_branches — set-algebra invariants (R10.3)
# ---------------------------------------------------------------------------


class TestComputeOrphanBranchesProperties:
    """Property-based invariants for ``compute_orphan_branches``.
    ...
    **Validates: Requirements 10.3**
    ...
    def test_pr_draftness_and_author_do_not_affect_orphan_decision(
        ...
        """Any PR (draft or not, bot or human) claims its source branch.

        :func:`compute_orphan_branches` only consults
        :attr:`PullRequest.source_branch` — flipping any other field
...
# ---------------------------------------------------------------------------
# compute_po_review_inbox — filter invariants (R10.4)
# ---------------------------------------------------------------------------


class TestComputePoReviewInboxProperties:
    """Property-based invariants for ``compute_po_review_inbox``.
...
# ---------------------------------------------------------------------------
# should_delete_branch — pure predicate invariants (R10.2)
# ---------------------------------------------------------------------------
#
# The retention predicate sits on the daily ``BotBranchRetention`` cron
# (task 2.4) and gates the deletion of stale ``ai/*`` branches.  Both
...
class TestShouldDeleteBranchProperties:
    """Property-based invariants for ``should_delete_branch``.
    ...
    **Validates: Requirements 10.2**
    ...
    def test_age_at_or_below_boundary_is_kept(
        ...
        reconsidered tomorrow.  This mirrors the requirement language
...
# ---------------------------------------------------------------------------
# iter_advance pure idempotency helpers (R10.1)
# ---------------------------------------------------------------------------
#
...
# The property test here pins the **pure** half of the contract:
#
...
# Temporal retries the activity (which is the loop the R10.1
# idempotency requirement actually guards against).
...
class TestIterAdvanceBannerProperties:
    """Property-based invariants for ``_build_banner`` (R10.1).
...
class TestDescriptionAlreadyBannersProperties:
    """Property-based invariants for ``_description_already_banners``.
    ...
    **Validates: Requirements 10.1**
    ...
    def test_idempotent_prepend_via_guard(
        ...
        is the property R10.1 actually requires from the production
        retry path.
