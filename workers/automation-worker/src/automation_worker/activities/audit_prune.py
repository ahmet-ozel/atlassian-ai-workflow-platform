"""Activities backing :class:`AuditPruneWorkflow` (Spec 3 task 13.2).
...
* :func:`notify_audit_prune_failed` — mandatory admin Slack alarm
  (R6.4) wired through :class:`notification.NotificationService`.
...
mandatory admin Slack alarm). The matching workflow contract lives in
:mod:`automation_worker.workflows.audit_prune` (Spec 3 task 13.1).

Dependency injection
--------------------

Activities are stateless functions decorated with ``@activity.defn``;
they read collaborators (Postgres pool, MinIO client,
:class:`NotificationService`) through module-level setters configured
...
Idempotence contract (Property 10 parity)
-----------------------------------------
...
#: created at MinIO boot by Spec 3 task 13.4 (``infra/minio/init.sh``);
...
#: Sibling task 13.4 owns the bucket lifecycle.
AUDIT_ARCHIVE_BUCKET: str = "audit-archive"

#: Number of audit rows fetched from Postgres per round-trip when
...
#: still produces a single object per day (the batches are concatenated
...
class _NotificationServiceLike(Protocol):
    ...
    the same object but is not consumed here. Sibling task 8.3 ships
...
def set_db_pool(pool: _AsyncPoolLike) -> None:
    ...
    Called once at worker boot (Spec 3 task 13.3 / ``main.py``) after
...
async def get_retention_setting() -> int:
    ...
    1. **Test override**: any callable installed via
       :func:`set_retention_setting_provider` is awaited; its return
    ...
    3. **``shared.feature_flags`` table** — the row whose
       ``name = audit_retention_days`` (constant
       :data:`RETENTION_FEATURE_FLAG_NAME`). The flag stores the value
    ...
    Notes:
        ...
        / DB value — the workflow body falls back to
        :data:`DEFAULT_RETENTION_DAYS` when this returns 0 / None
...
async def archive_audit_to_minio(cutoff: datetime) -> AuditArchiveResult:
    ...
    Returns:
        :class:`AuditArchiveResult` with ``archived_rows`` (count of
    ...
    Notes:
        ...
        payload data verbatim (R7.8 log-redaction parity).
    """
    pool = get_db_pool()
    ...
    async with pool.acquire() as conn:
        ...
        # batches sibling task 13.5 would switch to a server-side
...
async def delete_audit_older_than(cutoff: datetime) -> AuditDeleteResult:
    ...
    * ``DELETE FROM shared.cost_tracking    WHERE created_at < $1``
    ...
    Both tables share the same retention window per design.md
    ...
    Returns:
        :class:`AuditDeleteResult` with ``deleted_rows`` reflecting the
        number of rows removed from ``automation.audit_events``. Cost
        deletions are aggregated into the same counter only when they
        clearly belong to the audit slice; sibling task 13.5's
        property test asserts the audit-side count specifically, and
...
async def notify_audit_prune_failed(error_text: str) -> None:
    ...
    The activity is wired to :class:`notification.NotificationService`'s
    :meth:`notify_audit_prune_failed` (Spec 3 task 8.3) which renders
    ``prompts/notifications/audit_prune_failed.md`` and POSTs the body
    to the admin Slack channel via
    :meth:`SlackAdapter.send_admin_channel`. Reuses the existing
    :data:`notification.NotificationKind` ``"audit_prune_failed"``
    declared in :mod:`notification.types`.

    Args:
        error_text: Stringified exception forwarded by
            :class:`AuditPruneWorkflow` when the archive / delete
    ...
    Notes:
        * The activity does **not** swallow exceptions raised by the
          ...
          that propagates to Temporal. See
          :meth:`AuditPruneWorkflow._notify_failure`.
        * The "audit_prune_failed" admin alarm is mandatory per
          R6.4: it is the only signal an operator gets that the
...
def _encode_jsonl_gzip(rows: list[dict[str, Any]]) -> bytes:
    ...
    set produces a byte-identical archive — required for Property 10's
...
def _ensure_utc(value: datetime) -> datetime:
    ...
    ``datetime`` values in non-UTC zones are converted via
...
async def _minio_put_object(
    *,
    ...
    Raises:
        :class:`AuditArchiveTransportError` on transport / non-2xx
