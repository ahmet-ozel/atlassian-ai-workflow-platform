"""Webhook filter chain — tasks 4.1 + 4.2 of ``platform-mimari-workflows``.

This module hosts the **deterministic decision pipeline** for the
webhook gateway described in the workflows spec design document
(§"Webhook Filter Chain (deterministic decision)"). The full filter
stages (HMAC verification, dept resolution, loop guard, replay
deduplication, mention filter, first-iter exception, ``[bot:hear]``
bypass, comment-burst debounce) land across tasks **4.2 / 4.3 / 4.4**:

* Task 4.1 (this file's original skeleton) — data shapes
  (:class:`WebhookEvent`, :class:`FilterDecision`) and the chain
  orchestration object (:class:`WebhookFilterChain`).
* **Task 4.2 (this revision)** — ``verify_hmac``, ``resolve_dept``,
  ``loop_guard`` (with the ``^\\s*\\[bot:`` regex fallback when the
  actor account is missing from the payload). The two HTTP-level
  failure stages (HMAC invalid → 401, dept unresolved → 400) raise
  dedicated exceptions (:class:`WebhookHmacInvalidError`,
  :class:`WebhookDeptUnresolvedError`); the loop-guard drops surface
  as :class:`FilterDecision` verdicts.
* Task 4.3 — ``streamlit_bypass`` (the ``[bot:hear]`` etiquette tag
  from V12), ``replay_dedup``, ``mention_filter``, and the
  ``first_iter_exception`` (Z6) merged into the mention-filter
  decision so the two are evaluated atomically.
* Task 4.4 — the 3 second ``burst_debounce`` window that merges
  consecutive comments on the same issue into a single signal.
...
3. :class:`WebhookFilterChain` — the orchestration class. Its
   constructor accepts callbacks that the per-stage tasks compose into
   the actual filter logic. The :meth:`evaluate` method runs the
   stages in the order spelt out by the design's decision diagram:
   ...
   The skeleton supplied by task 4.1 returned a single
   ``filter_chain_skeleton`` pass for any input. Task 4.3 lights up
   the four mid-chain stages; tasks 4.2 / 4.4 will land the verifier
   stages and the burst-debounce stage in subsequent revisions
...
foundation spec's filter-chain contract (Property 3 in the
``platform-mimari-foundation`` test suite, foundation Requirement 4)
and keeps the chain testable with hypothesis-driven event sequences.

Requirements covered (workflows spec)
-------------------------------------

* **3.1** — Single normalised entry point for ``POST /webhooks/jira``
  and ``POST /webhooks/bitbucket`` (the dialect normalisation in
  :class:`WebhookEvent`).
* **3.2** — Jira event-type set (``jira:issue_created``,
  ``jira:issue_assigned``, ``jira:issue_updated``,
  ``jira:issue_commented``) is captured by the ``event_type`` field;
  the ``mention_filter`` stage only fires for ``issue_commented``.
* **3.3** — Bitbucket event-type set (``pullrequest:created``,
  ``pullrequest:commented``, ``pullrequest:updated``,
  ``pullrequest:fulfilled``) likewise lands in the same ``event_type``
  field; ``pullrequest:fulfilled`` is loop-guarded by stage 4.2.
* **3.4** — HMAC verification is delegated to the ``verify_hmac``
  callback whose runtime implementation reads
  ``vault:webhooks/<provider>/<dept_id>`` (foundation R6 V3).
* **3.5** — Dept resolution is delegated to ``resolve_dept``; the
  skeleton exposes a clean failure path for "dept could not be
  resolved" so the FastAPI router (task 4.5) can return HTTP 400.
* **4.3** — ``mention_filter`` drops ``issue_commented`` events whose
  actor is not in the bot-mentioned set and whose ``iter_count > 1``;
  audit reason is ``comment_ignored_unauthorized_actor``.
* **4.4** — ``first_iter_exception`` (Z6) bypasses the mention filter
  when ``iter_count == 1`` AND the actor matches the issue reporter;
  the chain returns ``pass`` with reason
  ``mention_filter_first_iter_exception``.
* **4.5** — ``streamlit_bypass`` (V12) detects the ``[bot:hear]``
  etiquette tag in the comment body and short-circuits to ``pass``
  with audit reason ``streamlit_inline_reply_with_bypass``. This
  bypass takes precedence over both ``replay_dedup`` and
  ``mention_filter``.

Note: the *behavioural* tests for these requirements (Property 3
``test_webhook_predicates.py``) live in tasks 4.9 / 4.10. The
unit-level coverage for task 4.3's mid-chain stages lives in
...
# ---------------------------------------------------------------------------
# Audit reason constants (task 4.3)
# ---------------------------------------------------------------------------
#
...
# task 4.5 and the unit / property tests) can match against the
...
#: ``[bot:hear]`` etiquette tag (V12). Streamlit's inline-reply UI
...
#: bypass applies). Maps to MIMARI §16.10.6 Y6 / requirement 4.3.
REASON_COMMENT_IGNORED_UNAUTHORIZED_ACTOR: Final[str] = (
    "comment_ignored_unauthorized_actor"
)

#: Audit reason emitted when the first-iter exception (Z6) bypasses
...
#: comment was honoured. Maps to MIMARI §16.11.1 Z6 / requirement 4.4.
REASON_MENTION_FILTER_FIRST_ITER_EXCEPTION: Final[str] = (
    "mention_filter_first_iter_exception"
)

#: Audit reason emitted when ``streamlit_bypass`` (V12) detects the
#: ``[bot:hear]`` tag and short-circuits the chain to ``pass``. Maps
#: to MIMARI §16.14.12 V12 / requirement 4.5.
...
# routing stage in task 4.6 — can validate against the same canonical
...
#: dropped** by the loop-guard stage (task 4.2) — it's a recognised
...
# ---------------------------------------------------------------------------
# Stage-specific constants — task 4.2 (HMAC, dept resolve, loop guard)
# ---------------------------------------------------------------------------
#
# These constants are exposed at module level so the FastAPI router
# (task 4.5) and the audit-log writer can both reference the same
...
# mid-chain ``REASON_*`` constants used by task 4.3
# (``REASON_DUPLICATE_EVENT_DROPPED``,
# ``REASON_COMMENT_IGNORED_UNAUTHORIZED_ACTOR``,
# ``REASON_MENTION_FILTER_FIRST_ITER_EXCEPTION``,
# ``REASON_STREAMLIT_INLINE_REPLY_WITH_BYPASS``) plus
# ``REASON_FILTER_CHAIN_PASS`` and ``REASON_BURST_COALESCED`` live with
# the task 4.3 / 4.4 declarations above; this block adds the task 4.2
# specific labels and the loop-guard fallback regex on top.

#: Loop-guard fallback regex (Requirements 4.2 / 4.6, design N15). When
...
#: Audit reason for HMAC verification failures (R3.4 / design table).
#: Surfaced through :class:`WebhookHmacInvalidError`; mapped to HTTP
#: 401 by the FastAPI router (task 4.5).
REASON_WEBHOOK_HMAC_INVALID: Final[str] = "webhook_hmac_invalid"

#: Audit reason for unresolved-dept rejections (R3.5 / design table).
...
#: Audit reason for actor-id loop-guard hits (R4.1, MIMARI §1 K7).
...
#: Audit reason for body-text loop-guard hits (R4.2 / R4.6, design
#: N15). Distinct from :data:`REASON_LOOP_GUARD_DROPPED` so operators
...
#: Audit reason emitted when the burst-debounce stage (task 4.4)
...
#: :class:`FilterDecision.coalesced_with`. Maps to MIMARI §16.6 /
#: requirement 4.7. Defined here (rather than in the task 4.3 block)
#: because the constant is referenced by ``_stage_burst_debounce``
#: which already exists in this file ahead of task 4.4's wiring.
REASON_BURST_COALESCED: Final[str] = "burst_coalesced"

#: Skeleton reason still emitted by the chain when no stage logic is
#: wired. Preserved for backwards compatibility with task 4.1's
#: signature contract; once tasks 4.2–4.4 land in full, no event
...
class WebhookHmacInvalidError(Exception):
    ...
    FastAPI router (task 4.5) can write a uniform audit row regardless
...
class FilterDecision:
    ...
    The decision is **structural** — it tells the FastAPI router (task
    4.5) what HTTP status to return and what audit action to log, but
    ...
    * :attr:`reason` — a short audit-friendly identifier matching the
      action labels in the design document's decision table. Known
...
# (task 4.1's verification) without an explicit class. The runtime
...
#: ids. Used by the mention-filter stage (task 4.3).
...
#: Used by the first-iter exception (R4.4).
ReporterFor = Callable[[str], str]


# ---------------------------------------------------------------------------
# Burst-debounce callback (task 4.4)
# ---------------------------------------------------------------------------
#
...
class BurstRegisterResult:
    ...
    * :attr:`decision` — ``"coalesce_emit"`` means the event opened a
      ...
      coalesced_with=...)`` per requirement 4.7.
    * :attr:`coalesced_with` — delivery_ids accumulated in the open
      window, including the dropped delivery itself (the design
      mandates "coalesced_with listesi delivery_id'leri içerir"). The
...
#: task 4.4; this constant is the canonical default that the chain
...
def normalize_bitbucket_event(
    *,
    ...
    if body_text is None:
        ...
        # for the ``[bot:`` regex fallback in task 4.2.
...
class WebhookFilterChain:
    ...
    Until tasks 4.2–4.4 land, :meth:`evaluate` returns the canonical
    skeleton decision::

        FilterDecision(action="pass", reason="filter_chain_skeleton")

    This is intentional: the FastAPI router (task 4.5) can already
    instantiate the chain and exercise the call-site without the
    behaviour drifting once the stage logic is wired in. The
    skeleton's *signature* is the contract that subsequent tasks
    must preserve.
    """
    ...
    def __init__(
        ...
        *,
        ...
        the design-document value of 3 seconds.
        ...
        resolve_dept:
            ...
            owns the project / repo (task 4.2 implementation).
        bot_account_ids:
        ...
        is_processed:
            ...
            replay-dedup stage in task 4.3 calls this with the event's
            ``delivery_id``.
        mention_set_for:
            Maps an ``issue_key`` to the set of accounts mentioned by
            the bot in that issue (task 4.3 implementation).
        iter_count_for:
            Returns the current iteration count for an ``issue_key``.
            Used by the first-iter exception (task 4.3).
        reporter_for:
            Returns the reporter ``account_id`` for an ``issue_key``;
            used by the first-iter exception (task 4.3).
        burst_window:
            Comment-burst debounce window (default 3 seconds, per
            design). The actual coalescing logic lives in task 4.4.
        burst_register:
            Optional adapter callback that wires the chain's
            ``_stage_burst_debounce`` stage (task 4.4) to a
            ...
            (task 4.5) instantiates the singleton ``BurstWindow``.
    ...
    # -----------------------------------------------------------------
    # Public properties — handy for tests and for the router (task 4.5)
    ...
    # -----------------------------------------------------------------
    # Stage helpers (task 4.3 — replay_dedup, mention_filter,
    ...
    # in lockstep with task 4.5; the test contract stays unchanged.

    # -----------------------------------------------------------------
    # Task 4.2 stages — verify_hmac, resolve_dept, loop_guard
    # -----------------------------------------------------------------
    #
    ...
    def _stage_verify_hmac(self, event: WebhookEvent) -> None:
        ...
        Implements requirement 3.4 / design table row "verify_hmac".
        ...
        rotation overlap window (foundation R6.8). The chain only
        ...
        (task 4.5) catches that exception, writes a
    ...
    def _stage_resolve_dept(self, event: WebhookEvent) -> None:
        ...
        Implements requirement 3.5 / design table row "resolve_dept".
        ...
        The router (task 4.5) catches that exception, writes a
    ...
    def _stage_loop_guard(
        ...
        Combined stage for requirements 4.1, 4.2, and 4.6 (MIMARI §1
        Kural 7, §14, §16.16 N15). The bot self-action loop guard
        ...
        1. **Actor-id match** — when ``actor_account_id`` is present
           ...
           the union, not on the per-dept registry. This matches the
           MIMARI §1 K7 contract and the foundation Property 4 / R1.7
           coverage.
        ...
        This mirrors the foundation Property 4 invariant that an
        empty registry never silently drops every webhook.
        ...
        # Actor is missing — fall back to the body-text regex (R4.2,
        # design N15). ``BOT_PREFIX_REGEX`` is anchored to the start
        # of the line with optional leading whitespace, so the
    ...
    def _has_streamlit_bypass_tag(body_text: str | None) -> bool:
        ...
        Implements the V12 etiquette tag detection used by
    ...
    def _stage_streamlit_bypass(
        ...
        """V12 — ``[bot:hear]`` tag bypasses the rest of the chain.
        ...
        diagram and in the design document's V12 row of the decision
        table.
    ...
    def _stage_replay_dedup(
        ...
        Idempotency anchor for requirement 4 / R3.5: the
    ...
    def _stage_mention_filter(
        ...
        """Y6 + Z6 — drop unauthorised comments unless the first-iter
        exception applies.

        Combined stage for requirements 4.3 and 4.4. The two checks
        are merged into a single method (rather than two stages in
        sequence) because the Z6 first-iter exception's purpose is
        explicitly to *bypass* the Y6 mention filter — running them
        as separate stages would risk reordering the precedence and
        either letting an unauthorised comment through (filter runs
        before exception) or dropping a legitimate first-iter comment
        (exception runs before filter and never sees iter > 1
        comments).
        ...
        1. **Scope** — only ``jira:issue_commented`` events are
           ...
           because both the Z6 reporter check and the Y6 mention-set
        ...
        2. **First-iter exception (Z6)** — if ``iter_count == 1``
           ...
           commented).
        3. **Mention filter (Y6)** — if ``iter_count > 1`` and the
        ...
        * ``0`` — the issue has never been processed; this should
          ...
          ``1`` first. We treat ``0`` the same as ``1`` for the Z6
          check so a freshly-created issue's first comment is
          honoured.
        * ``1`` — the workflow is in iter 1; Z6 applies if the
          actor matches the reporter.
        * ``≥ 2`` — Z6 does not apply and the mention filter is
          enforced.
        ...
        if event.issue_key is None:
            ...
            # we cannot evaluate Y6/Z6 deterministically, so we let
        ...
        # Z6 — first-iter exception. The reporter check runs first so
        ...
        # Y6 — mention filter. Only enforced for iter > 1 per the
        # decision table; iter == 1 with a non-reporter actor still
    ...
    def _stage_burst_debounce(
        ...
        """Task 4.4 — coalesce same-``issue_key`` events inside a 3s window.
        ...
        * No ``burst_register`` callback configured → ``None``
          (skip the stage; behaviour matches pre-task-4.4 chains).
        ...
        * Callback returns ``BurstRegisterResult(decision="coalesce_dropped")``
        ...
        Task 4.4 must observe the *real* deliveries that survived
    ...
    def evaluate(self, event: WebhookEvent) -> FilterDecision:
        ...
        ::
            ...
            verify_hmac        # task 4.2 — raises WebhookHmacInvalidError
            resolve_dept       # task 4.2 — raises WebhookDeptUnresolvedError
            loop_guard         # task 4.2 — actor-id + [bot: regex fallback
            streamlit_bypass   # task 4.3 — runs BEFORE replay_dedup
            replay_dedup       # task 4.3
            mention_filter     # task 4.3 — Z6 merged in
            burst_debounce     # task 4.4
            (default pass)
        ...
        matches the design document's filter-chain decision diagram.

        The stages owned by **task 4.3** are the four mid-chain
        filters that deal with comment-event policy: the V12
        ``[bot:hear]`` bypass, the replay-dedup against the
        ``processed_events`` table, and the Y6 mention filter with the
        Z6 first-iter exception merged in. Tasks 4.2 and 4.4 will land
        their stages as additional ``_stage_*`` methods and slot the
        calls into this orchestration without changing 4.3's logic.

        Why ``streamlit_bypass`` runs before ``replay_dedup``
        ----------------------------------------------------

        The V12 etiquette tag is the only signal that a comment came
        ...
        Why first-iter exception (Z6) lives inside mention_filter
        ---------------------------------------------------------

        Z6 is the *exception path* for the mention filter — its
        purpose is to let the reporter trigger the bot on iter 1
        before the bot has had a chance to mention anyone. Modelling
        Z6 as a separate stage in front of the mention filter would
        leak iter-aware logic into a position that runs for every
        event type, not just comments. Folding Z6 into the mention
        filter (as a precondition that returns ``pass`` when iter == 1
        and actor == reporter) keeps both rules co-located and makes
        ...
        # downstream stages (task 4.2 onwards) can assume the contract.
        ...
        # Task 4.2 stages — verify_hmac and resolve_dept raise
        ...
        # Task 4.3 stages — run streamlit_bypass first so [bot:hear]
        ...
        # Task 4.4 — burst debounce runs last so it only coalesces
