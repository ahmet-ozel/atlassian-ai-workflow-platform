"""CI gate — assistant_chat.md system prompt (`platform-mimari-ops` task 15.11).

**Validates: Requirements 1.4, 2.7**

The chat assistant system prompt at
``platform/prompts/assistant_chat.md`` (task 4.5) MUST:

* exist (task 4.5 ships it);
...
def test_prompt_file_exists() -> None:
    assert _PROMPT_PATH.is_file(), (
        f"Missing assistant_chat.md at {_PROMPT_PATH}. Task 4.5 ships "
        "this prompt; without it assistant-service cannot boot."
    )
...
def test_prompt_carries_y5_behaviour_sentence() -> None:
    """The Y5 / R1.3 mandatory sentence about write-action handoff."""

    body = _PROMPT_PATH.read_text(encoding="utf-8")
    # Both Turkish and English variants of the constraint trigger the
    # gate; the exact wording is task 4.5's call. We require every
    ...
    assert any(n in body for n in needles), (
        "assistant_chat.md must reference the Y5 / R1.3 contract — "
